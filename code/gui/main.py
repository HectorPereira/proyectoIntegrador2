...
# ===================== Config =====================
BAUD = 115200
SEND_INTERVAL_MS = 40     # frecuencia de envío durante arrastre
...
def parse_and_update(lines: str):
    """Procesa líneas y actualiza S1, Modo y el label de último mensaje."""
    last_line = None
    for line in lines.splitlines():
        ...
        if line.startswith("ANG="):
            try:
                ...
                servo_pos_var.set(f"S1: {ang_part}°")
...
# ======= Envío continuo mientras se mueve el slider =======
def _send_pending():
    send_after_id["id"] = None
    ...
    if deg != last_sent_deg["v"]:
        write_line(f"S:{deg}")
        last_sent_deg["v"] = deg
        last_msg_var.set(f"Último: > S:{deg}")
    if connected:
...
# Fila 1: Estado conexión + Último mensaje
...
servo_pos_var = tk.StringVar(value="S1: —°")
...
# Fila 3: Slider y botón de modo
angle_var = tk.IntVar(value=90)
ttk.Label(main, text="Ángulo (°)").grid(row=3, column=0, sticky="w", pady=(12,0))
