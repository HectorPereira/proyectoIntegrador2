...
\subsection{Código para el microcontrolador}\label{apendice:codigo_micro}

\subsubsection{Módulo UART: Inicialización y telemetría de servos}
...
\subsubsection{Parser de comandos UART (MODE, MAG, GON/GOFF, S1..S4)}
\begin{lstlisting}[language=C]
...
\subsubsection{Algoritmo de suavizado incremental (SLEW) para los servos}
\begin{lstlisting}[language=C]
...
\subsubsection{Generación de PWM por software (bit-banging) usando Timer1}
\begin{lstlisting}[language=C]
...
\subsubsection{Control del electroimán (MAG) con feedback mediante buzzer}
\begin{lstlisting}[language=C]
...
\subsubsection{Máquina de estados del buzzer para patrones de beeps}
\begin{lstlisting}[language=C]
...
\subsubsection{Antirrebote por software de los botones físicos}
\begin{lstlisting}[language=C]
...
\subsubsection{Bucle principal del firmware (control, PWM, telemetría)}
\begin{lstlisting}[language=C]
while(1){
    mode_button_poll();
    mag_button_poll();
...
\subsubsection{Módulo de conexión y comunicación serial (UART)}
...
\subsubsection{Módulo de interpretación de mensajes del firmware (Parser UART)}
\begin{lstlisting}[language=python]
...
\subsubsection{Envío progresivo de comandos desde los sliders (Rate Limiter)}
\begin{lstlisting}[language=python]
...
\subsubsection{Algoritmo de centrado suave de los cuatro servos}
\begin{lstlisting}[language=python]
...
\subsubsection{Grabación periódica de la trayectoria (Logger de poses)}
\begin{lstlisting}[language=python]
def record_tick():
...
\subsubsection{Reproducción de trayectorias con prealineado suave}
\begin{lstlisting}[language=python]
...
\subsubsection{Paso discreto de reproducción de la trayectoria}
\begin{lstlisting}[language=python]
def playback_step():
...
\subsubsection{Control del electroimán vía UART}
\begin{lstlisting}[language=python]
def toggle_magnet():
    global mag_state
    if not connected:
...
\subsubsection{Visualización 2D del brazo robótico (HUD cinemático)}
\begin{lstlisting}[language=python]
def update_hud():
